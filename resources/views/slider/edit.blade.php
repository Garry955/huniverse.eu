    ...
    <div class="mb-12 md:mb-0 lg:w-3/4">
        <form action="{{ route('slider.update', $slider->id) }}" method="POST" enctype="multipart/form-data">
               ...
               focus:border-primary focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150"
                ...
                <div
                    ...
                    <span class="text-gray-400 w-full overflow-hidden m-0 pt-[56.25%] relative">
                        <img id="preview"
                            src="{{ $slider->image ? asset('/storage/slider/' . $slider->image) : '#' }}"
                            alt="slider_image"
                            class="absolute top-1/2 left-1/2 bg-cover translate-x-[-50%] translate-y-[-50%]"
            ...
            <div class="relative mb-6" data-twe-input-wrapper-init>
                <input type="text" value="{{ $slider->lead ? $slider->lead : old('lead') }}"
            ...
            <!-- text input -->
            <div class="relative mb-6" data-twe-input-wrapper-init>
                <textarea maxlength="255" rows="5" cols="50"
                    style="background-color: light-dark(rgb(232, 240, 254), rgba(70, 90, 126, 0.4)) !important;"
                    class="peer block min-h-[auto] w-full rounded border-0 bg-transparent px-3 py-[0.32rem] leading-[2.15] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 peer-focus:text-primary data-[twe-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none dark:text-white dark:placeholder:text-neutral-300 dark:autofill:shadow-autofill dark:peer-focus:text-primary [&:not([data-twe-input-placeholder-active])]:placeholder:opacity-0"
                    id="text" placeholder="Szöveg" name="text">{{ $slider->lead ? $slider->lead : old('lead') }}</textarea>
                </textarea>
                <label for="text"
                    class="pointer-events-none absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.37rem] leading-[2.15] text-neutral-500 transition-all duration-200 ease-out -translate-y-[1.15rem] peer-focus:scale-[0.8] peer-focus:text-primary peer-data-[twe-input-state-active]:-translate-y-[1.15rem] peer-data-[twe-input-state-active]:scale-[0.8] motion-reduce:transition-none dark:text-neutral-400 dark:peer-focus:text-primary">
                    Szöveg
            ...
            <div class="text-center lg:text-left">
                <button type="submit"
                    class="inline-block w-1/2 lg:w-1/3 rounded bg-primary lg:px-16 pb-2 pt-3 text-sm font-medium uppercase leading-normal text-white shadow-primary-3 transition duration-150 ease-in-out hover:bg-primary-accent-300 hover:shadow-primary-2 focus:bg-primary-accent-300 focus:shadow-primary-2 focus:outline-none focus:ring-0 active:bg-primary-600 active:shadow-primary-2 dark:shadow-black/30 dark:hover:shadow-dark-strong dark:focus:shadow-dark-strong dark:active:shadow-dark-strong"
                    data-twe-ripple-init data-twe-ripple-color="light">
                    <i class="fa-regular fa-square-plus text-xl mb-[-1px] mr-2"></i>Dia szerkesztése </button>
            </div>
        </form>
        ...
        <div class="text-center lg:text-left">
            <form action="{{ route('slider.destroy', $slider->id) }}" method="POST">
                ...
                @method('delete')
                <button type="submit"
                    class="inline-block w-1/2 lg:w-1/3 rounded bg-red-500 text-center px-7 py-2 mt-5 text-sm font-medium uppercase leading-normal text-white shadow-primary-3 transition duration-150 ease-in-out hover:bg-red-600 hover:shadow-primary-2 focus:bg-red-accent-300 focus:shadow-primary-2 focus:outline-none focus:ring-0 active:shadow-primary-2 dark:shadow-black/30 dark:hover:shadow-dark-strong dark:focus:shadow-dark-strong dark:active:shadow-dark-strong"
                    data-twe-ripple-init data-twe-ripple-color="light">
                    Dia törlése
                </button>
            </form>
